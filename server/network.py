...
def loadData(raw_data):
    ...
    # person_matrix = pickle.loa d(person_pkl)
    ...
    people_dataset = CocoDataset(images,
                             transform=torchvision.transforms.Compose([
                                   torchvision.transforms.Resize((28, 28)),
                                   torchvision.transforms.ToTensor(),
                                   torchvision.transforms.Normalize((0.1307, ), (0.3081,))
    ...
    batch_size_train = 60
    people_data_loader = torch.utils.data.DataLoader(
        people_dataset,
        batch_size=batch_size_train,
        shuffle=True
    )
...
def train(device, epoch):
    ...
    batch_idx = -1
    for (data, target) in people_data_loader:
        batch_idx += 1
        ...
        print('Train Epoch: {} [{}/{} ({:.2f}%)]\tLoss: {:.6f}'.format(
            epoch, batch_idx * len(data), len(people_data_loader.dataset),
            100. * batch_idx / len(people_data_loader), loss.item()
        ))
